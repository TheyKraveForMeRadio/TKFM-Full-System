...
echo "== TKFM: Engine CTA SMART LOCK (won't break if a lane price isn't configured) =="
echo "What this does:"
echo " - Keeps the PRO CTA bars + Neon styling"
echo " - Tries to LOCK each engine page to the intended lane planId"
echo " - BUT if the required STRIPE_PRICE_* env var is missing, it FALLS BACK to the page's first existing data-plan"
echo "   (so checkout never breaks again)"
echo

STAMP="$(date +%Y%m%d_%H%M%S)"
BK="backups/engine-cta-smart-lock-$STAMP"
mkdir -p "$BK"
...
declare -A INTENDED_PLAN
declare -A REQUIRED_ENV

INTENDED_PLAN["video-engine.html"]="video_monthly_visuals"
REQUIRED_ENV["video-engine.html"]="STRIPE_PRICE_VIDEO_MONTHLY_VISUALS"

INTENDED_PLAN["social-engine.html"]="social_starter_monthly"
REQUIRED_ENV["social-engine.html"]="STRIPE_PRICE_SOCIAL_STARTER_MONTHLY"

INTENDED_PLAN["press-engine.html"]="press_run_pack"
REQUIRED_ENV["press-engine.html"]="STRIPE_PRICE_PRESS_RUN_PACK"

INTENDED_PLAN["podcast-engine.html"]="podcast_monthly_boost"
REQUIRED_ENV["podcast-engine.html"]="STRIPE_PRICE_PODCAST_MONTHLY_BOOST"

declare -A PILL TITLE COPY LIST
...
COPY["video-engine.html"]="Checkout for the lane, submit your link, and we push it through the TKFM pipeline: review → scheduling → featured placements."
LIST["video-engine.html"]=$'        <li><strong>Submit:</strong> YouTube/Vimeo link or Drive link (share access)</li>\n        <li><strong>We deliver:</strong> Featured windows + rotation opportunities</li>\n        <li><strong>Fast follow-up:</strong> confirmation + scheduling notes</li>'
...
COPY["social-engine.html"]="This is a paid lane. Checkout, submit your campaign, and we build a clean push that feeds TKFM visibility."
LIST["social-engine.html"]=$'        <li><strong>Submit:</strong> goal + links + target city + notes</li>\n        <li><strong>We deliver:</strong> a posting plan + featured options</li>\n        <li><strong>Upgrades:</strong> Rotation Boost + Homepage Feature</li>'

PILL["press-engine.html"]="TKFM PRESS ENGINE • PAID PRESS RUN"
TITLE["press-engine.html"]="Press that looks official — and gets seen."
COPY["press-engine.html"]="Buy the pack, submit your materials, and we run the press pipeline: review → formatting → placement plan → delivery."
LIST["press-engine.html"]=$'        <li><strong>Submit:</strong> press link / EPK / one-sheet + socials</li>\n        <li><strong>We deliver:</strong> a structured press run plan + assets</li>\n        <li><strong>Goal:</strong> credibility + exposure that converts</li>'

PILL["podcast-engine.html"]="TKFM PODCAST ENGINE • PAID FEATURED LANE"
TITLE["podcast-engine.html"]="Get your episode featured — not buried."
COPY["podcast-engine.html"]="Checkout, submit your episode link, and we slot it into TKFM’s Featured rail + rotation. Clean, simple, effective."
LIST["podcast-engine.html"]=$'        <li><strong>Submit:</strong> episode link + show name + guest + release date</li>\n        <li><strong>Placement:</strong> Featured player on TKFM TV + promo windows</li>\n        <li><strong>Optional:</strong> short intro clip for maximum clicks</li>'

pick_fallback_plan () {
  local f="$1"
  grep -oE 'data-plan="[^"]+"' "$f" | head -n1 | sed 's/^data-plan="//; s/"$//' || true
}

env_price_is_set () {
  local var="$1"
  local val
  val="$(netlify env:get "$var" --context production 2>/dev/null | tr -d '\r' | tr -d '\n' || true)"
  [[ "$val" == price_* ]]
}
...
<section class="tkfmCtaWrap" role="region" aria-label="TKFM Paid Lane">
  <div class="tkfmCtaTop">
    <div>
      <div class="tkfmCtaPill">${PILL[$file]}</div>
      <div class="tkfmCtaH">${TITLE[$file]}</div>
      <div class="tkfmCtaCopy">${COPY[$file]}</div>
...
for f in "${PAGES[@]}"; do
  ...
  plan="${INTENDED_PLAN[$f]}"
  req="${REQUIRED_ENV[$f]}"

  if env_price_is_set "$req"; then
    chosen="$plan"
    note="LOCKED"
  else
    fb="$(pick_fallback_plan "$f")"
    if [ -n "${fb:-}" ]; then
      chosen="$fb"
      note="FALLBACK (missing $req)"
    else
      echo "WARN: $f has no data-plan and $req not set. Skipping CTA insert."
      continue
    fi
  fi

  INS="$(mk_cta_block "$f" "$chosen")"
