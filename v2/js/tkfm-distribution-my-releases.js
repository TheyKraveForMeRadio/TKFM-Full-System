(function(){
  ...
  function getEmail(){
    let e = '';
    try{ e = localStorage.getItem(KEY_EMAIL) || ''; }catch(err){}
    e = (e||'').trim().toLowerCase();
    return e;
  }

  function setEmail(e){
    try{ localStorage.setItem(KEY_EMAIL, e); }catch(err){}
  }

  function renderList(items){
    if(!Array.isArray(items) || !items.length){
      host.innerHTML = '<div class="mini">No releases found for this email yet.</div>';
      return;
    }
    host.innerHTML = items.slice(0,80).map(it=>{
      const id = esc(it.id);
      const title = esc(it.project_title||'Untitled');
      const artist = esc(it.primary_artist||it.name||'');
      const status = esc((it.status||'submitted').toUpperCase().replace(/_/g,' '));
      const when = esc((it.updated_at||it.created_at||'').slice(0,19).replace('T',' '));
      return (
        '<div class="card2" style="margin-top:10px">'+
          '<div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;align-items:center">'+
            '<div><div style="font-weight:950">'+title+'</div><div class="mini">'+artist+'</div></div>'+
  ...
  async function load(){
    ...
    if(!email){
      host.innerHTML = '<div class="warn">Enter your email to load your releases.</div>';
      return;
    }
    ...
    if(!out.ok || !out.data || !out.data.ok){
      host.innerHTML = '<div class="warn">Load failed. '+esc(out.text)+'</div>';
      return;
    }
    renderList(out.data.items || []);
  }
