...
function allowInsecureLocal(event){
  if(!insecureFlagEnabled()) return false;
  if(!isLocalHost(event)) return "MISCONFIG";
  return true;
}

function localDefaultLookupKey(){
  return String(process.env.TKFM_WEBHOOK_LOCAL_DEFAULT_STUDIO_LOOKUP_KEY || "").trim();
}

function localDefaultCustomerId(){
  return String(process.env.TKFM_WEBHOOK_LOCAL_DEFAULT_CUSTOMER_ID || "").trim();
}
...
async function lookupKeyFromSession(stripe, sessionId){
...
async function lookupKeyFromInvoice(stripe, invoiceId){
...
export async function handler(event){
  if(event.httpMethod === "GET"){
    return json(200, { ok:true, endpoint:"stripe-webhook-studio", hint:"POST from Stripe. GET sanity-check only." });
  }
  ...
  if(insecure === true){
    ...
    try{
      ...
      recordDelivery("studio", { id:"(unknown)", type:"bad_signature", error: e.message, insecure:false });
  ...
  try{
    ...
    if(type === "checkout.session.completed"){
      lookupKey = (insecure === true) ? localDefaultLookupKey() : await lookupKeyFromSession(stripe, obj.id);
      if(insecure === true && !customerId) customerId = localDefaultCustomerId();
    ...
    if(type === "invoice.paid" || type === "invoice_payment.paid" || type === "invoice.payment_succeeded"){
      lookupKey = (insecure === true) ? localDefaultLookupKey() : await lookupKeyFromInvoice(stripe, obj.id);
      if(insecure === true && !customerId) customerId = localDefaultCustomerId();
      const res = award(customerId, lookupKey, stripeEvent.id || "(noid)");
      awarded = res.awarded; credits = res.credits;
    }
