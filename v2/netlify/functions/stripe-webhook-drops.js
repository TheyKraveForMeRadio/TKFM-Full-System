...
const PLAN_CREDITS = {
  drop_pack_10:10, drop_pack_25:25, drop_pack_100:100,
  ai_drops_starter_monthly:10, ai_drops_pro_monthly:25, ai_drops_studio_monthly:100,
  radio_imaging_bundle:25, custom_voice_setup:10, custom_voice_hosting_monthly:25
};
...
export async function handler(event){
  if(event.httpMethod === "GET"){
    return json(200, { ok:true, endpoint:"stripe-webhook-drops", hint:"POST from Stripe. GET is sanity-check only." });
  }
  ...
  if(insecure === true){
    ...
    catch(e){ recordDelivery("drops", { id:"(unknown)", type:"bad_signature", error:e.message, insecure:false }); return json(400, { ok:false, error:"BAD_SIGNATURE" }); }
  }
