...
export async function handler(event) {
  ...
  let session;
  try {
    session = await stripe.checkout.sessions.retrieve(session_id, { expand: ["line_items.data.price"] });
  } catch (e) {
  ...
  if (!lookup || !ALLOWED_LOOKUP.has(lookup)) return bad(400, "not_distribution_plan", { lookup_key: lookup || null });

  const name = trim(draft.name, 80);
  const email = trim(draft.email, 160).toLowerCase();
