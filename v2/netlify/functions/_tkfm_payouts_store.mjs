...
const FILE = path.join(DATA_DIR, "payouts.json");

function ensureDir(){ try{ fs.mkdirSync(DATA_DIR,{recursive:true}); }catch(e){} }
export function nowISO(){ return new Date().toISOString(); }
export function uid(prefix="pay"){ return prefix + "_" + crypto.randomBytes(6).toString("hex"); }
...
export function loadPayouts(){
  const j = readFile();
  globalThis.__TKFM_PAYOUTS_STORE = j;
  return j;
}

export function savePayouts(j){
  try{
    writeFile(j);
    globalThis.__TKFM_PAYOUTS_STORE = j;
